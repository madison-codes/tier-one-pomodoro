...
Timer.prototype.noise = function() {
  let endTime = new Audio('/sounds/end-time.wav');
  let intervalTime = new Audio('/sounds/interval-time.wav');
  if (this.remaining < 21000 && this.remaining > 20000) { intervalTime.play(); }
  if (this.remaining < 16000 && this.remaining > 15000) { intervalTime.play(); }
  if (this.remaining < 11000 && this.remaining > 10000) { intervalTime.play(); }
  if (this.remaining < 6000 && this.remaining > 5000) { intervalTime.play(); }
  if (this.remaining < 1000 && this.remaining > 0) { endTime.play(); }
};
