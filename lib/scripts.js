...
function setCustomBreak() {
  ...
  else { console.log('Please enter a valid rest time.');}
}
...
function setCustomFocus() {
  ...
  if (isValidInput(custom)) { return custom; }
  else { console.log('Please enter a valid focus time.');}
}
...
$resumeButton.on('click', function() {
  showPauseButton();
  Pomodoro.get();
  // Pomodoro.createWorkTimer(10);
  // Pomodoro.pomodoros[1].start();
  // Pomodoro.pomodoros[1].changeStateToRunning();
  // Pomodoro.pomodoros[1].end();
  // var seconds = Pomodoro.pomodoros[1].remaining() / 1000;
  // Pomodoro.pomodoros[1].remaining();
  // showPauseButton();
  // countDown(seconds, Pomodoro.pomodoros[1]);
  Pomodoro.pomodoros[0].state = 'resume pressed';
